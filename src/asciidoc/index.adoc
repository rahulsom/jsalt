...
[source,scala]
----
include::../main/scala/FactorialScala.scala[]
----
...
<4> This is a basic demo of type inference.
<5> This is a method body with `{}`.
<6> Also Scala infers the return of this method. It is not `Object`.
<7> ScalaJs is very aggressive in optimizing generated javascript. We need to tell it what we will be using from
  outside unless we have a `JSApp` with a `main` method.
<8> More demo of advanced type inference.
<9> This looks like an expression, but it is a function.

[source,scala]
----
include::../test/scala/FactorialScalaSpec.scala[]
----
